<?php
    ...
    if($_SERVER['REQUEST_METHOD'] == "GET"){
        ...
        if($name != "" && $payDate != "" && $mobile1 != "" && $user1 != "" && $payType != "" && $payment != ""){
            $query = "SELECT * FROM institute WHERE stat = '1' AND name = ? OR stat = '2' AND name = ?";
            ...
            if($res->num_rows > 0){
                ...
                $stmt2->bind_param("sssssssss", $name, $payDate, $payType, $mobile1, $user1, $mobile2, $user2, $payment, $date);
                if($stmt2->execute()){
                    echo 1;
                }else{
                    echo 2;
